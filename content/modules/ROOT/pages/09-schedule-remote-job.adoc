...
following. This will target the image mode host `+rhel2+` and update it to the new container image stored in the Satellite container registry.
...
=====
.Check the status of the image mode host
[%collapsible]
====
...
=====
.Reboot into the new container image
[%collapsible]
====
In the `rhel2.lab terminal`, enter the following to reboot into the new container image.
